...
export function SettingsTab({ event, eventId }: SettingsTabProps) {
  ...
  const archiveMutation = trpc.events.archive.useMutation({
    onSuccess: () => { toast.success("Event archived."); router.push("/dashboard"); },
  ...
  const deleteMutation = trpc.events.delete.useMutation({
    onSuccess: () => { toast.success("Event deleted."); router.push("/dashboard"); },
  ...
  const getStatusBadge = () => {
    ...
    if (s === "published") return <Badge className="rounded-full px-3 text-[10px] font-bold uppercase bg-green-500/10 text-green-500 border border-green-500/20">Live</Badge>;
    if (s === "completed") return <Badge className="rounded-full px-3 text-[10px] font-bold uppercase bg-blue-500/10 text-blue-500 border border-blue-500/20">Completed</Badge>;
    if (s === "archived") return <Badge className="rounded-full px-3 text-[10px] font-bold uppercase bg-slate-500/10 text-slate-500 border border-slate-500/20">Archived</Badge>;
    return <Badge className="rounded-full px-3 text-[10px] font-bold uppercase bg-amber-500/10 text-amber-500 border border-amber-500/20">Draft</Badge>;
  };
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <CardDescription className="text-xs mt-0.5">Control the status and state of your event</CardDescription>
              </div>
            </div>
            {getStatusBadge()}
          </div>
